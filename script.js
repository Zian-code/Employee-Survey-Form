...
function showWarning(message, questions) {
    ...
    warning.innerHTML = `Please select one of these options (${questions.length} remaining)`;
...
function validatePage() {
    ...
    if (!isValid) {
        showWarning('Please select one of these options', unansweredQuestions);
    }
    return isValid;
}
...
function renderPage() {
    ...
    if (page.questions) {
        page.questions.forEach(q => {
            html += `<label>${q.label}<span class="required-field">*</span></label>`;
